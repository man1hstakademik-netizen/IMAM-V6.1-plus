...
const BottomNav: React.FC<BottomNavProps> = ({ currentView, onNavigate, userRole }) => {
  const role = userRole || UserRole.TAMU;
  const canScanQr = canAccessPermission(role, Permission.SCAN_QR);
  ...
  const navItems = [
    ...
    { id: 'reports', view: ViewState.REPORTS, label: 'Laporan', icon: ChartBarIcon },
    { id: 'profile', view: ViewState.PROFILE, label: 'Akun', icon: UserIcon },
  ].filter((item) => canAccessView(role, item.view));
      ...
      <div className="pointer-events-auto relative w-full max-w-md px-4 pb-2">
        <div className="absolute bottom-24 right-6 z-50 flex flex-col items-center gap-4">
          ...
          <div className="flex flex-col items-center gap-1 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className={`px-2.5 py-0.5 rounded-full shadow-lg border border-white/20 ${canScanQr ? 'bg-indigo-600 dark:bg-indigo-500' : 'bg-slate-400 dark:bg-slate-600'}`}>
              <span className="text-[7px] font-black text-white uppercase tracking-[0.2em]">Scan QR</span>
            </div>
            <button
              onClick={() => canScanQr && onNavigate(ViewState.SCANNER)}
              disabled={!canScanQr}
              className={`w-12 h-12 rounded-2xl text-white shadow-[0_10px_20px_rgba(79,70,229,0.4)] flex items-center justify-center border-2 border-white/20 transition-all duration-300 group overflow-hidden ${
                canScanQr
                  ? 'bg-gradient-to-br from-indigo-600 via-blue-600 to-indigo-700 active:scale-90 hover:-translate-y-1'
            ...
            return (
              ...
              >
                <div className={`relative p-2 rounded-xl transition-all duration-500 ${
                  isActive ? 'bg-indigo-50 dark:bg-indigo-500/10 -translate-y-1 shadow-inner' : 'group-hover:bg-slate-50 dark:group-hover:bg-slate-800/50'
                }`}>
                  <Icon className={`w-5 h-5 transition-all duration-500 ${isActive ? 'stroke-[2.5px] scale-110' : 'stroke-[1.5px]'}`} />
                </div>
                <span className={`text-[8px] font-black uppercase tracking-wider transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-70'}`}>
                  {item.label}
                </span>
              </button>
            );
