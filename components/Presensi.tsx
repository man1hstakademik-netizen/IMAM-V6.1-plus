...
const Presensi: React.FC<{ onBack: () => void, onNavigate: (v: ViewState) => void }> = ({ onBack, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    if (isMockMode) {
        setTimeout(() => {
            setAllStudents([
                { id: '1', namaLengkap: 'Adelia Sri Sundari', tingkatRombel: 'XII IPA 1', nisn: '0086806447', status: 'Aktif' } as any
            ]);
            setLoading(false); 
        }, 500);
        return;
    ...
    db.collection("students").where("status", "==", "Aktif").onSnapshot(s => setAllStudents(s.docs.map(d => ({ id: d.id, ...d.data() } as Student))));
      ...
      <div className="bg-white/80 dark:bg-[#0B1121]/90 backdrop-blur-xl px-5 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 safe-pt z-40">
          <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <p className="text-[10px] font-medium text-indigo-600 mt-1.5">Tampilan spreadsheet</p>
              </div>
          </div>
          ...
          <div className="p-4 lg:p-6 space-y-6 pb-32">
              <div className="bg-white dark:bg-[#151E32] p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-4">
                  ...
                  <div className="relative flex-1 group">
                      ...
                      <input 
                          type="text" placeholder="Filter nama..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                          className="w-full pl-11 py-3 bg-slate-50 dark:bg-slate-900 border-none rounded-xl text-[10.5px] font-medium focus:ring-2 focus:ring-indigo-500/20"
                      />
                  </div>
                  <button onClick={() => setIsHaidMode(!isHaidMode)} className={`px-5 py-3 rounded-xl text-[10.5px] font-bold border transition-all ${isHaidMode ? 'bg-rose-600 text-white border-rose-400 animate-pulse' : 'bg-white dark:bg-slate-800 text-slate-400'}`}>
                      ...
                      <table className="excel-table">
                          ...
                          <tbody>
                              {loading ? (
                                  ...
                                  <tr><td colSpan={9} className="py-20 text-center text-slate-400 font-medium">Tidak ada data</td></tr>
                              )}
                          </tbody>
                      </table>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-900/50 p-3 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center text-[10px] font-bold text-slate-400">
                      <span>Total: {displayData.length} records</span>
                      <span className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div> Online</span>
                  </div>
              </div>
          </div>
      </div>
